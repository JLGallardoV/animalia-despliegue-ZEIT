...
export class FuncionamientoBitacoraService {
  ...
  constructor(public http:HttpClient) { }
    ...
    public agregarAcceso(accionAcceso:string,idUsuario: number) {
      ...
      this.http.post('/accesosWS/agregarAcceso', {accionAcceso,idUsuario}, { headers: this.headers }).subscribe(
        (success:any)=>{
    ...
    public registrarAcceso(accion:string,nombreUsuario: string) {
      this.http.get(`/usuariosWS/buscarUsuarioPorNombre/${nombreUsuario}`, { headers: this.headers }).subscribe(
        (success: any) => {
